...
switch at the same time, with each element being optional. Each
wired/wireless LAN client can communicate with each other. IPAM is
handled via `dnsmasq`, providing DCHP and DNS to all clients: the LAN IP
range, as well as initial reservations are customizable.

WAN connectivity is expected to be provided via an authenticated PPPoE
link. `iptables` rules then control forwarding between the LAN and the
...
the full documentation before trying to run it, as misconfiguring the
...
The overall structure is quite simple: the `anssible/routerify.yaml`
file contains the plays, which simply calls on roles from
`ansible/roles/`.

Almost all configuration is done via Ansible variable. Most of them are
kept within the `defaults` of the role to which they pertain logically.
A few variables which didn't fit nicely in any roles where placed in
...
example of this are `hostapd` configuration files. Changing the SSID or
the AP channel can be done via variables, but lower-level settings can
only be changed by editing the files.

## How to use

* First, install a base Debian 10 system. There are no particular
  constraints, so you can partition the disks as you like and install
  any additional tools you want. If planning to run the playbook on the
  system itself, Ansible must be installed. Otherwise, if a remote
...
create wireless AP's or wired LAN's, it merely expects the kernel to
support them. If it is not the case, drivers/firmware must be installed
before running the playbook.

However, routerify was written to automate a specific deployment
scenario which involved a well-defined network adapter, employing the
...
you the effort to install it manually. It also include some nice module
options tweaks, for example to enable 80MHz channels in 802.11ac mode.

If using any other WiFi chip, disable this feature.
...
* `ap24_interface_name`: how the interface used for the 2.4GHz AP will be
  renamed;
...
* `ap5_interface_name`: how the interface used for the 5GHz AP will be
  renamed;
...
be enabled or disabled on its own. It's called by other AP roles.
...
inherited by `ap24` and `ap5` unless overridden. Useful to set the same
...
* `wired_lan_interface_name`: the final name the chosen LAN interface
  will be renamed to;
* `wired_lan_interface_mac`: the interface is selected by its MAC
  address in colon format (i.e. `00:11:22:33:44:55`).
...
* `lan_bridge_dhcp_range_end:`: the last address of the DHCP
  pool, inclusive (i.e. `10.0.1.199`);
* `lan_bridge_dhcp_reservations`: a list of MAC reservations. Eac
...
Currently, it is not possible to disable DHCP, but hosts can still be
...
* the screen shutdowns after a timeout while displaying the console.
...
* `powersave_tty_poweroff_minutes`: controls the inactivity timeout, in
  minutes, after which the screen will power off, when showing the
  system console.
...
* `wan_ppp_interface_name`: final name of the PPP interface. _Currently
  renaming this interface does not work, so please __do not override__
  this variable_.
...
* inbound traffic that is related to already seen outgoing traffic is
  allowed;
* establishing new connections from the WAN is not allowed (i.e. all
  ports are closed).
...
* `routing_other_rules` defines ports on the router itself that will be
  accessible _from any interface which is not the PPP interface (this
  means that such ports are open for additional, non PPPoE traffic
  coming from the WAN interface)_. This is mainly useful if you need to
...
As it is, the `Vagrantfile` is incomplete. It will try to load ad
...
For both the 2.4GHz and 5GHz AP's, one should provide their MAC
